...
只不过可惜的是虚拟主机不能支持 https，所以无奈使用了`COS`。
...
>
> 腾讯云官方给我们提供了一个解决方案，可以在 COS 存储桶的函数计算->CDN 缓存刷新函数中配置一个函数，参考截图：
...
>
> 优化方案
> 可以通过简单修改官方的函数来解决这个问题，点击刚刚创建的 CDN 缓存刷新函数列表中的函数名称，将 index.js 文件内容替换为下面的代码，最后点击右上角的“部署”按钮即可：
...
exports.main_handler = async (event, context) => {
	 ...
	 */
	const { secretId, secretKey, token, objects, triggerType, cdnHosts } =
		getParams(event);
	...
	results.forEach((item) => {
		...
		if (
			item.params.urls[0].lastIndexOf('/index.html') ==
			item.params.urls[0].length - 11
		) {
